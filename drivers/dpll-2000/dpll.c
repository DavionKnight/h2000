...
int idt285_read(unsigned short addr, unsigned char *data)
{
	unsigned short wdata = 0, bufaddr = 0x400;	
	unsigned char slot = slot_num, mode = 1;
	...
	wdata = bufaddr;
	fpga_read_once(slot, I2C_RDEN, mode,&wdata);
	wdata = (wdata^0x1)&0x1;
	...
	do{
		wdata = bufaddr;
		fpga_read_once(slot, I2C_EN, mode, &wdata);
...
int idt285_write(unsigned short addr, unsigned char data)
{
	unsigned short wdata = 0, bufaddr = 0x400;	
...
//			printf("Write:%02x %02x\n",regaddr,val);
			idt285_write(regaddr, val);
	usleep(10);
			idt285_read(regaddr,&data);
//			printf("Read:%02x %02x\n",regaddr,data);
		}
		memset(reg_buf,0,20);
		fret = fgets(reg_buf,20,fp);
