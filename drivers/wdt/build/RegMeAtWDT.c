 ...
 * @version <pre> $Id: RegMeAtWDT.c,v 1.2 2005/06/08 11:55:05 sst Exp $ </pre>
 * 
...
int main(int argc, const char *argv[]) {
	...
	// Serve Watchdog for Task (in time every 6 sec (at least every 20 secons is needed as registered) 
    serve_a_task(fd, my_pid, 5, 6); 		// Do service to that task 5 times every 6 Seconds

    // Serve Watchdog for Task delayed 25sec (at least every 20 secons is needed as registered) 
    serve_a_task(fd, my_pid, 1, 25); 		// Do service to that task 1 (once) after 25 sec (5 sec delayed)

	// Unregister Task with current PID
	unregister_a_task(fd, my_pid);
