...
int mpls_fragment(struct sk_buff *skb, int max_frag_size)
{
   ...
   */
  int               shim_size = 0;
  struct rtable    *rt = (struct rtable *)skb->_skb_refdst;  /* by tianzhy 2015-06-03*/
  int               err = 0;
  u32               common_to_all;
  ...
  while (left > 0)
	...
	len = mtu;
	 ...
	 then align the next start on an eight byte boundary */
      ...
      if (skb->sk)
	skb_set_owner_w(skb2, skb->sk);
      skb2->_skb_refdst = (unsigned long)dst_clone(skb_dst(skb));  /* changed by tianzhy 2015-06-03*/
      skb2->dev = skb->dev;
