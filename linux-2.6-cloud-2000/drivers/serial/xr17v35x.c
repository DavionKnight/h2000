...
*
*      Based on Linux 2.6.21 Kernel's  drivers/serial/8250.c and /8250_pci.c
*
...
*
*
*	   Multiport Serial Driver for EXAR's PCI Family of UARTs (XR17V358/354/352/8358/4358/8354)
*	   for			: LINUX 2.6.32	(Tested with Ubuntu 10.04)
*	   date			: Sep, 2010
*	   version		: 1.2
*
		  ...
		  struct uart_port *port, int idx)
{
	...
	{
	    offset += 0x1000; // the ports on expansion device for 0x(4/8)354 sit at baar0+0x2000 offset. So we need to 0x1000 here as 4*0x400
	}
...
static irqreturn_t serialxr_interrupt(int irq, void *dev_id)
{
	...
	struct list_head *l;
	unsigned char gisr = 0, slave_gisr = 0;
	int portcount, j;
	...
	gisr = serial_in(up, 0x80);

	l = i->head;
	portcount = 0;
	while(gisr && (portcount < 8))
	{
	    if(gisr & int_offset[portcount])
	    {
	      up = list_entry(l, struct uart_xr_port, list);
	...
	l = i->head;
	up = list_entry(l, struct uart_xr_port, list);
	if(up->deviceid > 0x358) // for ports on expansion slot
	{
	    switch(up->deviceid)
	    {	      
	      case 0x4358:
		...
		break;
	      ...
	      default:
		printk(KERN_ERR "serialxr: incorrect deviceid - 0x%x\n", up->deviceid);
	    }
	    up = list_entry(l, struct uart_xr_port, list); // this must be the first port on the expansion interface
	    //read global status register of the device on expansion slot
	    slave_gisr = serial_in(up, 0x80);
	}
	
	portcount = 0;
	while(slave_gisr && (portcount < 8))
	{   
	    if(slave_gisr & int_offset[portcount])
	    {
	      up = list_entry(l, struct uart_xr_port, list);
	      ...
	      slave_gisr &= ~(int_offset[portcount]);
	    }
...
static int serialxr_startup(struct uart_port *port)
{
	...
	*/
	...
	// extra for XR17v35x
	up->ier |= XR_17V35X_IER_RTSDTR | XR_17V35X_IER_CTSDSR;
	serial_out(up, UART_IER, up->ier);
...
void pcieserial_remove_ports(struct serial_private *priv)
{
	struct pci_serial_quirk *quirk;
	int i;
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Exar PCIe specific serial driver for XR17V35x- Revision: 1.2");
