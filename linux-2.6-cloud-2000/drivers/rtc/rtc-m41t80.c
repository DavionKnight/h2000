...
#endif
/*****Added by suipl 2013.3.4 for JH1022***********/
#ifdef CONFIG_P1022_DS
	/* disable SQW, set SQW frequency to 0 */
    rc = i2c_smbus_read_byte_data(client, M41T80_REG_ALARM_MON);
	if (rc < 0)
		return -EIO;
    if (i2c_smbus_write_byte_data(client, M41T80_REG_ALARM_MON,
				      rc & ~M41T80_ALMON_SQWE) < 0)
		return -EIO;
	rc = i2c_smbus_read_byte_data(client, M41T80_REG_WDAY);
	if (rc < 0)
		return -EIO;
	rc = (rc & 0x0f);
	if (i2c_smbus_write_byte_data(client, M41T80_REG_WDAY, rc) < 0)
		return -EIO;
#endif
/**************End*********************************/
	return 0;
