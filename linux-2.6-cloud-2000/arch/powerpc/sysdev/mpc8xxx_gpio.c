...
static int mpc8xxx_gpio_to_irq(struct gpio_chip *gc, unsigned offset)
{
	...
	//printk("mpc8xxx_gpio.c!!!!!!!!!!!irq = %d\n", (int)mpc8xxx_gc->irq->inval_irq);
...
//	struct mpc8xxx_gpio_chip *mpc8xxx_gc = get_irq_desc_data(desc);
	...
	desc1 = irq_to_desc(irq);
	mpc8xxx_gc = get_irq_desc_data(desc1);
	mm = &mpc8xxx_gc->mm_gc;
	//printk("mpc8xxx_gpio.ccc!!!!!!!!!!!irq = %d\n", irq);
	//printk("mpc8xxx_gpio.ccc!!!!!!!!!!!irq = %d\n", (int)mpc8xxx_gc->irq->inval_irq);
...
#if 0
static irqreturn_t gpio_test(int irq, void *dev_id)
{
        int  handled = 0;
...
//	int rrr;
	...
	//printk("mpc8xxx_gpio.c: ...........irq = %d\n", (int)mpc8xxx_gc->irq->inval_irq);
/*
        rrr=request_irq(hwirq,gpio_test,0,"mpc8xxx-gpio", NULL);
        if(rrr < 0)
